...
char ssid[] = "Alfheim";                 // your network SSID (name)
char pass[] = "asu7Kiri10";             // your network password
int status = WL_IDLE_STATUS;          // the Wifi radio's status
byte mac[] = { 0x74, 0x90, 0x50, 0x00, 0x79, 0x03 };
IPAddress ip(172, 20, 10, 99);
...
#define SPEED_MARGIN 0.01
#define LINEAR_MAX 0.7
#define ANGULAR_MAX 0.4
#define LINEAR_RATE ( MAX_SPEED / LINEAR_MAX )
#define ANGULAR_RATE ( MAX_SPEED / ANGULAR_MAX )
...
void on_topic(uxrSession* session, uxrObjectId object_id, uint16_t request_id, uxrStreamId stream_id, struct ucdrBuffer* mb, void* args) {
    ...
    if( (topic.linear.x > SPEED_MARGIN) || (topic.linear.x < -1.0 * SPEED_MARGIN)){
      // Forward or Backword
      int speed = LINEAR_RATE * topic.linear.x;
      motors.setRightSpeed(speed);
      motors.setLeftSpeed(speed);
    } else if(topic.angular.z > SPEED_MARGIN){
      // CCW
      int speed = abs(ANGULAR_RATE * topic.angular.z);
      motors.setRightSpeed(speed);      
      motors.setLeftSpeed(-1 * speed);
    } else if(topic.angular.z < SPEED_MARGIN){
      // CW
      int speed = abs(ANGULAR_RATE * topic.angular.z);
      motors.setRightSpeed(-1 * speed);      
      motors.setLeftSpeed(speed);
    } else {
